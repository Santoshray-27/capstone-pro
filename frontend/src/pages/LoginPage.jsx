...
import { FileText, Sparkles, Eye, EyeOff, Loader, ArrowLeft, BarChart3, Zap, Users, Mail, Lock, UserSearch, Cpu } from 'lucide-react';
            ...
            <form onSubmit={handleSubmit} className="w-full space-y-6">
              ...
              <div>
                ...
                <div className="mt-2 text-left">
                   <a href="#" className="text-[14px] font-semibold text-[var(--primary)] hover:opacity-70 transition-opacity">Forgot Password?</a>
                </div>
              </div>

              <button type="submit" disabled={loading}
                className="w-full bg-[var(--primary)] text-white py-4 rounded-xl font-semibold text-[16px] shadow-xl shadow-[var(--primary)]/20 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2">
                {loading ? <><Loader size={18} className="animate-spin" /> Loging in...</> : 'Log In'}
              </button>
            </form>

            <div className="mt-8 flex flex-col items-center gap-6 w-full">
              <Link to="/register" className="text-[15px] font-medium text-[var(--muted-foreground)]">
                New to ResumeXpert AI? <span className="text-[var(--primary)] font-semibold hover:underline">Create an account.</span>
              </Link>
              ...
              <button 
                ...
                className="flex items-center gap-2 text-[14px] font-semibold text-muted-foreground hover:text-[var(--foreground)] transition-colors py-2 px-4 rounded-lg hover:bg-muted"
              >
      ...
      <div className="hidden lg:flex lg:w-[40%] relative overflow-hidden bg-card">
        <div className="absolute inset-0 m-4 rounded-[40px] overflow-hidden flex flex-col items-center justify-center text-white p-12 shadow-2xl"
          ...
          <div className="relative z-10 max-w-xs text-center mb-16">
            <h2 className="text-4xl font-bold leading-tight mb-6 tracking-tight">Let AI power your next career move.</h2>
            <p className="text-lg font-normal opacity-90 leading-relaxed">Log in to access your AI-powered dashboard and accelerate your professional growth.</p>
          </div>
            ...
            <div className="p-6">
               ...
               <div className="grid grid-cols-3 gap-3 mb-6">
                  <div className="p-3 rounded-2xl bg-muted border border-border">
                    ...
                    <div className="text-lg font-bold text-foreground mt-0.5">87%</div>
                  </div>
                  <div className="p-3 rounded-2xl bg-muted border border-border">
                    <div className="w-7 h-7 rounded-lg bg-emerald-100 flex items-center justify-center mb-2">
                      <Zap size={18} className="text-emerald-600" />
                    </div>
                    <div className="text-[9px] font-bold text-muted-foreground uppercase tracking-wider">Insight</div>
                    <div className="text-lg font-bold text-foreground mt-0.5">High</div>
                  </div>
                  <div className="p-3 rounded-2xl bg-muted border border-border">
                    <div className="w-7 h-7 rounded-lg bg-amber-100 flex items-center justify-center mb-2">
                      <FileText size={18} className="text-amber-600" />
                    </div>
                    <div className="text-[9px] font-bold text-muted-foreground uppercase tracking-wider">Docs</div>
