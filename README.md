...
## 🌐 Live Demo
- **App URL**: https://5000-i33fm70f7jcv8jxikg8pr-583b4d74.sandbox.novita.ai
- **API Health**: https://5000-i33fm70f7jcv8jxikg8pr-583b4d74.sandbox.novita.ai/api/health
...
# backend/.env
PORT=5000
MONGODB_URI=mongodb://localhost:27017/smart-resume-analyzer
...
3. **Set environment variables** in Railway dashboard:
   ```
   MONGODB_URI=mongodb+srv://...@cluster.mongodb.net/smart-resume-analyzer
...
3. Get connection string → `MONGODB_URI=mongodb+srv://...`
4. Whitelist IPs or use `0.0.0.0/0` for Railway
...
### User
- `name`, `email`, `password` (hashed), `role` (jobseeker/recruiter/admin)
- `profile`: phone, location, bio, skills, LinkedIn, GitHub
- `stats`: resumes uploaded, analyses count, interviews completed

### Resume
- File metadata (name, type, size, path)
- `parsedData`: name, email, skills[], experience[], education[]
- `rawText`, `atsScore`, `status`

### Analysis
- `atsScore`, `scoreBreakdown` (6 dimensions)
- `strengths[]`, `weaknesses[]`, `missingSkills[]`, `suggestions[]`
- `keywordsFound[]`, `keywordsMissing[]`, `sectionFeedback`

### Interview
- `questions[]`: question, type, difficulty, expectedAnswer, userAnswer, evaluation
- `overallScore`, `readinessLevel`, `topStrengths[]`, `areasToImprove[]`
...
| File upload 400 | Only PDF/DOCX files under 10MB accepted |
| AI analysis returns mock data | Add `GEMINI_API_KEY` or `OPENAI_API_KEY` to `.env` |
| CORS error | Set `FRONTEND_URL` in backend `.env` to match Vercel URL |
